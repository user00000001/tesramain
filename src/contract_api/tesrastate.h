...
using plusAndMinus = std::pair<dev::u256, dev::u256>;  //tesra commmetn the first: the plus value;the second:the minus value
...
class TesraState : public dev::eth::State
{
...
public:
    ...
    TesraState(dev::u256 const &_accountStartNonce, dev::OverlayDB const &_db, const std::string &_path,
              dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute
    execute(dev::eth::EnvInfo const &_envInfo, dev::eth::SealEngineFace const &_sealEngine, TesraTransaction const &_t,
...
private:
    ...
    dev::Address createTesraAddress(dev::h256 hashTx, uint32_t voutNumber);
...
{
    std::unique_ptr<TesraState> &globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<TesraState> &_globalStateRef) :
...
public:
    ...
    CondensingTX(TesraState *_state, const std::vector<TransferInfo> &_transfers, const TesraTransaction &_transaction,
...
private:
    ...
    const TesraTransaction &transaction;
