...
//    dev::g_logPost(std::string("\n\n\n\n\n\n\n\n\n\n"), NULL);
    ...
    //dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::tesraMainNetwork)));
...
uint64_t GetMinGasPrice(int height)
{
    ...
    TesraDGP tesraDGP(globalState.get(), fGettingValuesDGP);
    //globalSealEngine->setTesraSchedule(tesraDGP.getGasSchedule(height));
                     ...
                     string &errinfo, const CAmount nAbsurdFee, bool rawTx)
{
    ...
    LogPrintStr("CheckContractTx"); //tesra debug
                   ...
                   ByteCodeExecResult &testExecResult)
{
    ...
    ExtractTesraTX resultConverter;
    LogPrintStr("RunContractTx"); //tesra debug
                            ...
                            int &level, string &errinfo,uint64_t &countCumulativeGasUsed,uint64_t &blockGasUsed)
{
    ...
    // LogPrintf("before TesraTxConverter: vtx addr %p\n", &block.vtx); //tesra debug
    //LogPrintf("before TesraTxConverter: vtx size %d\n", block.vtx.size()); //tesra debug
...
void GetState(uint256 &hashStateRoot, uint256 &hashUTXORoot)
{
    ...
    dev::h256 oldHashStateRoot(globalState->rootHash()); // tesra-vm
    dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // tesra-vm
...
bool ByteCodeExec::processingResults(ByteCodeExecResult &resultBCE)
{
    for (size_t i = 0; i < result.size(); i++)
    {
        ...
        {
            //LogPrintStr("TransactionException != None"); //tesra debug
            if (txs[i].value() > 0)
            { //refund the value to sender
               ...
               // LogPrintf("*****222:: gas[%d]:%ld\n",i,CAmount(txs[i].value())); //tesra debug
...
//            LogPrintf("*****:: amount[%d]:%ld\n",i,amount); //tesra debug
        ...
        {
            LogPrint("processingResults ","%d", i); //tesra debug
...
dev::Address ByteCodeExec::EthAddrFromScript(const CScript &script)
{
    ...
    if (ExtractDestination(script, addressBit, &txType))
    {
                ...
                addressBit.type() == typeid(CKeyID))
        {
            ...
            LogPrint("ByteCodeExec::EthAddrFromScript ","%s", HexStr(addr.begin(), addr.end())); //tesra debug
