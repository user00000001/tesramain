...
public:
    ...
    TesraDGP(TesraState *_state, bool _dgpevm = true) : dgpevm(_dgpevm), state(_state)
    {
        initDataEIP158();
    }
...
private:
    ...
    const TesraState *state;

    dev::Address templateContract;

    std::map<dev::h256, std::pair<dev::u256, dev::u256>> storageDGP; //tesra hashedKey,<key,value>

    std::map<dev::h256, std::pair<dev::u256, dev::u256>> storageTemplate;//tesra hashedKey,<key,value>

    std::vector<unsigned char> dataTemplate;   //tesra call contract ,res->output
