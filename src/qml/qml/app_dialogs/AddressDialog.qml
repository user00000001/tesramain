...
CommonDialog
{
    ...
    Item{
        ...
        CommonTextField
        {
            ...
            anchors.left: parent.left
            anchors.leftMargin: 82
            placeholderText: "Enter a TESRA address (e.g. u7VFR83SQbiezrW72hjcWJtcfip5krte2Z)"
    ...
    onConfirmed:
    {
        if(forSending)
        {
            if(!walletModel.validateAddress(addressTextField.text))
            {
                ...
                root_window.warningDialog.content_text = qsTr("Please input validate TESRA address")
