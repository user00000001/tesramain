...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::TSR, nValue);
...
{
    std::string genesisblockhash = "2ec285d71217777d5753c03834420a6fe992f74679b80853f5a7ce87f6aef609";
...
BlockExplorer::BlockExplorer(QObject *parent) : QObject(parent)
...
bool BlockExplorer::isTxindexSet()
{
    return GetBoolArg("-txindex", false);
//    if (m_NeverShown) {
//        m_NeverShown = false;
...
//            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (tesra.conf).");
//            QMessageBox::warning(this, "TESRA Core Blockchain Explorer", Warning, QMessageBox::Ok);
//        }
//    }
}


void BlockExplorer::startUpPage()
{
    CBlockIndex* pindexBest = mapBlockIndex[chainActive.Tip()->GetBlockHash()];
...
void BlockExplorer::onSearch(const QString& query)
{
    goTo(query);
}
...
{
    QString CSS = "body {font-size:14px; color:#f8f6f6; bgcolor:#12546a;} a, span { font-family: monospace; } span.addr {color:#12546a; font-weight: bold;} table tr td {padding: 3px; border: 1px solid black;} td.d0 {font-weight: bold; color:#f8f6f6;} h2, h3 { white-space:nowrap; color:#12546a;} a { color:#88f6f6; text-decoration:none; } a.nav {color:#12546a;} ";

    //QString CSS = "body {font-size:12px; color:#f8f6f6; bgcolor:#12546a;} a, span { font-family: monospace; } span.addr {color:#12546a; font-weight: bold;} table tr td {padding: 3px; border: 1px solid black; background-color: #12546a;} td.d0 {font-weight: bold; color:#f8f6f6;} h2, h3 { white-space:nowrap; color:#12546a;} a { color:#88f6f6; text-decoration:none; } a.nav {color:#12546a;} p { background-color: #12546a; }";
    QString FullContent = "<html><head><style type=\"text/css\">" + CSS + "</style></head>" + "<body>" + Content.c_str() + "</body></html>";
    // printf(FullContent.toUtf8());

    //qDebug()<<"FullContent:"<<FullContent;


//    QMessageBox mBox(QMessageBox::Critical, "test", FullContent, NULL);
//    mBox.setFixedWidth(800);
//    mBox.setFixedHeight(800);

//    mBox.setTextFormat(Qt::RichText);
//    mBox.setTextInteractionFlags(Qt::LinksAccessibleByMouse|Qt::TextSelectableByMouse);

//    mBox.exec();


   emit newContent(FullContent);
}
...
void BlockExplorer::back()
{
    ...
    if (0 <= NewIndex && NewIndex < m_History.size()) {
        ...
        emit newSearch(m_History[NewIndex]);
...
{
    emit setBackButton(m_HistoryIndex - 1 >= 0);
    emit setForwardButton(m_HistoryIndex + 1 < m_History.size());
}
