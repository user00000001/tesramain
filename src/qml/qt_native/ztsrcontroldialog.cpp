...
#include "ui_ztsrcontroldialog.h"
...
std::list<std::string> ZTsrControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTsrControlDialog::listMints;

ZTsrControlDialog::ZTsrControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTsrControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTsrControlDialog::~ZTsrControlDialog()
{
    delete ui;
}

void ZTsrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTsrControlDialog::updateList()
{
...
void ZTsrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTsrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTsr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTsrControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTsrControlDialog::GetSelectedMints()
{
...
void ZTsrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
