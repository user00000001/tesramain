                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTSR ought to be enough for anybody." - Bill Gates, 2017
    ui->zTSRpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTSRSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTSR->setEnabled(false);
        ui->pushButtonMintzTSR->setToolTip(tr("zTSR is currently disabled due to maintenance."));

        ui->pushButtonSpendzTSR->setEnabled(false);
        ui->pushButtonSpendzTSR->setToolTip(tr("zTSR is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTSRpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTSR_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTSR is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTSR...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTSR in ") + 
...
void PrivacyDialog::on_pushButtonSpendzTSR_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTSR is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTSR
        sendzTSR();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTSR
    sendzTSR();
}

void PrivacyDialog::on_pushButtonZTsrControl_clicked()
{
    ZTsrControlDialog* zTsrControl = new ZTsrControlDialog(this);
    zTsrControl->setModel(walletModel);
    zTsrControl->exec();
}

void PrivacyDialog::setZTsrControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTsrSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTSR()
{
    ...
    double dAmount = ui->zTSRpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTSR is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TSR </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTSR</b>";
    ...
    // use mints from zTsr selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTsrControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZTsrControlDialog::GetSelectedMints();
    }
    ...
    // Clear ztsr selector in case it was used
    ZTsrControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTsr Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Tsr, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTsr Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTSR </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTSR "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTSR "));
    ui->labelzTSRAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTSRSyncStatus->setVisible(fShow);
}
