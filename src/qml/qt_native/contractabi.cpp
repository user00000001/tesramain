...
#include <univalue.h>
#include <contract/libethcore/ABI.h>
#include <math.h>
...
bool ParameterABI::abiOutBasic(ParameterType::Type abiType, const std::string &data, size_t &pos, std::string &value) const
{
    ...
    case ParameterType::abi_bytes:
    {
		...
		dev::string32 outData = dev::eth::ABIDeserialiser<dev::string32>::deserialise(o);
    ...
    try
    {
        ...
        if(decodeType().isDynamic() && !decodeType().isList())
        {
            ...
            case ParameterType::abi_string:
            {
                dev::bytes rawData = dev::fromHex(data.substr(pos));
                dev::bytesConstRef o(&rawData);
                value.push_back(dev::eth::ABIDeserialiser<std::string>::deserialise(o,pos));
            }
                break;
            default:
                m_lastError = UnsupportedABI;
...
QString ParameterABI::getRegularExpession(const ParameterType &paramType)
{
    ...
    case ParameterType::abi_bytes:
    {
        ...
        else
        {
            ...
            return QString(paternBytes32).arg(paramType.totalBytes()*2);
