...
std::string toStringHash160(const CWallet* wallet, const CTxDestination& dest)
{
    ...
    CBitcoinAddress txSenderAdress(dest);
    ...
    if (wtx.IsCoinStake()) {
        ...
        for (const CTxOut txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime);
                sub.type = TransactionRecord::ZerocoinSpend_Change_zTsr;
        ...
        if (fAllFromMeDenom && fAllToMeDenom && nFromMe * nToMe) {
            ...
            if(wtx.HasCreateOrCall()){
                ...
                if(ExtractDestination(pwallet->mapWallet[wtx.vin[0].prevout.hash].vout[wtx.vin[0].prevout.n].scriptPubKey, address))
                {
                    sub.address = toStringHash160(wallet, address);
                }
