...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the TSR and zTSR money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 38802, 38804));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in TSR/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in TSR/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, tesra, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in TSR/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:38802"));
    ...
    strUsage += HelpMessageOpt("-backupztsr=<n>", strprintf(_("Enable automatic wallet backups triggered after each zTsr minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizetesraamount=<n>     " + strprintf(_("Keep N TSR anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 38803, 38805));
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-blockaddress")) {
		...
		if (!address.IsValid()) {
			InitError(_("Invalid TESRA address for -blockaddress=<address>"));
			return false;
		}
		newBlockAddress = address;
	}
...
#endif
    // Make sure only a single TESRA process is using the data directory.
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                if (fReindex){
                    //TODO:tesra-evm ,fReindex is true ,wipeResults
                ...
                // TESRA: load previous sessions sporks if we have them.
...
//                }
                ...
                //checked with tesra codes. tesra-vm
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZTSRMinted();
                        RecalculateZTSRSpent();
                    }
                    RecalculateTSRSupply(1);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZTsrBackups = GetBoolArg("-backupztsr", true);
        pwalletMain->setZTsrAutoBackups(fEnableZTsrBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeTesraAmount = GetArg("-anonymizetesraamount", 0);
//    if (nAnonymizeTesraAmount > 999999) nAnonymizeTesraAmount = 999999;
//    if (nAnonymizeTesraAmount < 2) nAnonymizeTesraAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1TSR+1000 == (.1TSR+100)*10
       10TSR+10000 == (1TSR+1000)*10
    */
