...
{
	static std::string deserialise(bytesConstRef& io_t, int pos = 0)
	{
		pos /= 2;
		unsigned o = (uint16_t)u256(h256(io_t.cropped(0, 32)));
		assert(o >= pos);
		const unsigned int strLenStart = o - pos;
		unsigned s = (uint16_t)u256(h256(io_t.cropped(strLenStart, 32)));
		std::string ret;
		ret.resize(s);
		io_t.cropped(strLenStart + 32, s).populate(bytesRef((byte*)ret.data(), s));
