...
void VM::interpretCases()
{
    ...
    DO_CASES
    {
        ...
        CASE(TESRAINFO)
        {
            ...
            Address _owner = asAddress(m_SP[0]);
            u256   _need = u256(m_SP[1]);
            h256   _bytes = h256(m_SP[2]);
            u256   _height = u256(m_SP[3]);
            std::vector<uint8_t> _value;
            bool ret;
            u256 result = u256(0);

            ret = getExtendedData((uint32_t)_height, (char*)_bytes.data(), _type, _value, _owner);
            ...
            m_SPP[0] = result << 8 * (32 - _value.size());
        }
        NEXT
