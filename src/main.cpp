...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zTsr spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zTsr spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zTsr spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < 80000 && nHeight >= 0) {
        nSubsidy = 21920 * COIN;
    } else if (nHeight <= 159999 && nHeight >= 80000) {
        nSubsidy = 10960 * COIN;
    } else if (nHeight <= 239999 && nHeight >= 160000) {
        nSubsidy = 5480 * COIN;
    } else if (nHeight <= 319999 && nHeight >= 240000) {
        nSubsidy = 2740 * COIN;
    } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 320000) {
        nSubsidy = 1370 * COIN;
    } else if (nHeight <= 57164800 && nHeight > Params().LAST_POW_BLOCK()) {
        nSubsidy = 685 * COIN;
        
        int halvings = (nHeight - (Params().LAST_POW_BLOCK() + 1)) / 1576800;
        ...
        {
            nSubsidy *= (1 + 0.01 * ((nHeight - 1 + chainActive[nHeight - 1]->nTime)% 11));
        }
    } else {
        nSubsidy = 0 * COIN;
    }
...
void RecalculateZTSRSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zTSR supply
...
bool RecalculateTSRSupply(int nHeightStart)
{
...
{
    // TESRA: recalculate Accumulator Checkpoints that failed to database properly
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //-----------------tesra-vm-----------------------
    //Note: No need to add tesraDGP here, Will exam in ContractTxConnectBlock
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        bool hasOpSpend = tx.HasOpSpend(); //tesra-vm
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zTsr with serial %s is already in the block %d\n",
    ...
    //LogPrintf("ConnectBlock(): prev supply %s, nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zTsrSpent: %s\n", FormatMoney(nMoneySupplyPrev),
...
//            }
        ...
        for (size_t i = 0; i < checkVouts.size(); i++) {
            ...
            //LogPrintf("*****:: checkVouts[%d].scriptPubKey:%s\n",i,checkVouts[i].scriptPubKey.ToString()); //tesra debug
           // LogPrintf("*****:: checkVouts[%d].nValue:%ld\n",i,checkVouts[i].nValue); //tesra debug
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert TSR to zTSR
...
bool static ConnectTip(CValidationState& state, CBlockIndex* pindexNew, CBlock* pblock, bool fAlreadyChecked)
{
    ...
    LogPrintf("bench - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        if (!rv) {
            ...
            UpdateState(oldHashStateRoot, oldHashUTXORoot);//tesra-vm
            ClearCacheResult();//tesra-vm
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if(block.IsContractEnabled()){
            ...
            //Don't allow contract opcodes in coinbase   //tesra-vm
    ...
    bool lastWasContract = false; //tesra-vm
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zTsr spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zTsr serial %s in block\n Block: %s",
...
bool ProcessNewTmpBlockParam(CTmpBlockParams &tmpBlockParams, const CBlockHeader &blockHeader)
{
    ...
    if(CheckProofOfWork(blockHeader.GetPoWHash(), blockHeader.nBits) && !tmpblockmempool.HaveTmpBlock(tmpBlockParams.GetHash())) {
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zTsr mints and %d zTsr spends\n", __func__, nMints, nSpends);
...
bool static LoadBlockIndexDB(string& strError)
{
    ...
    pblocktree->ReadFlag("logevents", fLogEvents);//tesra-vm
    LogPrintf("logevents %s", fLogEvents ? "enabled" : "disabled");//tesra-vm
...
bool CVerifyDB::VerifyDB(CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    uint256 oldHashStateRoot, oldHashUTXORoot;
    GetState(oldHashStateRoot, oldHashUTXORoot);//tesra-vm
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // TESRA: We use certain sporks during IBD, so check to see if they are
...
{
    //if (fDebug)
    //    LogPrintf("ProcessMessages(%s),(%u messages),(%x version)\n", pfrom->addrName.c_str(),pfrom->vRecvMsg.size(),pfrom->nVersion);
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        // get next message
        CNetMessage& msg = *it;
//        if(it->hdr.GetCommand().compare("version")==0)
            LogPrintf("ProcessMessage[%s],(%s),(%08X)\n",pfrom->addrName,it->hdr.GetCommand().c_str(),pfrom->nVersion);
        //if (fDebug)
