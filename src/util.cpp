...
static void DebugPrintInit()
{
    ...
    assert(fileoutVM == nullptr); // tesra-vm
    ...
    boost::filesystem::path pathDebugVM = GetDataDir() / "vm.log"; // tesra-vm
    fileoutVM = fopen(pathDebugVM.string().c_str(), "a");// tesra-vm
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "tesra" is a composite category enabling all TESRA-related debug output
...
//        }
   ...
   // fwrite("what happened?\n", 1, sizeof("what happened?\n"), stdout);
    ...
    if (fPrintToConsole) {
        ...
        fflush(stdout);
    } else if (fPrintToDebugLog) {//tesra-vm
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\TESRA
// Windows >= Vista: C:\Users\Username\AppData\Roaming\TESRA
// Mac: ~/Library/Application Support/TESRA
// Unix: ~/.tesra
#ifdef WIN32
    // Windows
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty tesra.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override tesra.conf
